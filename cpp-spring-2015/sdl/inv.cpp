...
int main(int argc, char* args[]) {
    ...
    // Create Window
    window = SDL_CreateWindow("Inv", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, s.w, s.h, SDL_WINDOW_SHOWN);
    if (window == NULL) {
...
void init_state(State &s, int stage) {
    ...
    for (int i = 0; i < ww; i++) {
        for (int j = 0; j < hh; j++) {
            ...
            s.enemies[index].b.x += rand() % (enemy_size - size + 1) / 2  *
                                    (rand()%2 * 2 - 1);
        }
    }
    ...
    for (int i=0; i < s.N; i++) {
        if (s.enemies[i].alive) {
            ...
            if (s.enemies[i].shotdown) {
                ...
                s.enemies[i].b.x += 100.0 * ((s.enemies[i].b.x < s.w/2 ? -1 : 1) + 0.2 * z) * dt;
            }
            else {
                // normal enemies
